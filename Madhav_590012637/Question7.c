...
void addGoalToStadium0(int minute, int playerID) {
    if (goalCount[0] >= MAX_GOALS) {
        printf("Stadium 0 is full\n");
        return;
    }
...
void addGoalToStadium2(int minute, int playerID) {
    ...
    goals[2][goalCount[2]] = minute;   // FIX: was goals[1][...]
    playerGoals[playerID]++;
    goalCount[2]++;
}
...
int totalGoals() {
    int total = 0;
    for (int s = 0; s < STADIUMS; s++) {   // FIX: was s <= STADIUMS
        total += goalCount[s];
    }
    return total;
}
...
int topScorer() {
    int maxG = 0, winner = 0;
    for (int p = 0; p < PLAYERS; p++) {    // FIX: was p = 1
...
void displayStadium(int stadium) {
    for (int i = 0; i < goalCount[stadium]; i++) {   // FIX: was i <= goalCount[stadium]
...
int main() {
    // Sample data to match Expected Output
